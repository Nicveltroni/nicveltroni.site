...
(async function init() {
  ...
  function applyTheme(name) {
    ...
    const logoText = document.querySelector('.logo-text, .hud__logo-text');
    if (logoText) logoText.src = name === 'light'
      ? 'logos/SVG/viliya logo text black.svg'
      : 'logos/SVG/viliya logo text white.svg';
  ...
  if (toggle) {
    ...
    toggle.addEventListener('click', () => {
      ...
      if (stageEl) {
        ...
        const oldFilter = document.body.classList.contains('theme-light') ? 'url(#metallic-light)' : 'url(#metallic)';
        stageClone.querySelectorAll('.logo-text').forEach(el => el.style.filter = oldFilter);
        stageClone.querySelectorAll('svg:not(.spray)').forEach(el => el.style.filter = oldFilter);
